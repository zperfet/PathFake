# 主要用于weibo数据的预处理；对已有数据进行一定的整合和转换
# 输入：
# 1）标签 id:label 2）文本 id:text 3)树文件
# 输出：
#
...
weibo_id_label_path = join(data_dir_path, 'weibo_id_label.txt')
weibo_id_text_path = join(data_dir_path, 'weibo_id_text.txt')
weibo_tree_dir_path = join(data_dir_path, 'TreeWeibo')
weibo_all_path_ids_path = join(data_dir_path, 'weibo_all_path_ids.txt')
weibo_all_path_raw_texts_path = join(data_dir_path, 'weibo_all_path_raw_texts.txt')
weibo_all_path_texts_path = join(data_dir_path, 'weibo_all_path_texts.txt')

weibo_all_response_raw_texts_path = join(data_dir_path, 'weibo_all_response_raw_texts.txt')
weibo_all_response_texts_path = join(data_dir_path, 'weibo_all_response_texts.txt')
...
def generate_path_ids_and_texts(tree_dir_path, path_ids_save_path, path_texts_save_path):
    ...
    id2text_dict = get_text_dict(weibo_id_text_path)
    path_sum = 0
    all_path_ids = []
    all_path_texts = []
...
def dataset_split(ids, fold_save_dir_path, k_fold):
    ...
    for ki in range(k_fold):
        fold_train_path = join(fold_save_dir_path, 'TrainWeiboFold%d.txt' % ki)
        fold_test_path = join(fold_save_dir_path, 'TestWeiboFold%d.txt' % ki)
...
# 中文分词
def chinese_tokenization(lines_lst):
    lines_token = []
    print('开始分词,总共', len(lines_lst))
    for cnt, line in enumerate(lines_lst):
        if cnt % 1000 == 0:
            print("分词句数：", cnt)
        new_line = ' '.join(jieba.cut(line))
        lines_token.append(new_line)
    return lines_token
...
if __name__ == '__main__':
    ...
    # 划分数据集,五重交叉验证
    # print('划分微博数据集...')
    # weibo_id_labels = load_lst(weibo_id_label_path)
    # ids = [line.split('\t')[0] for line in weibo_id_labels]
    # dataset_split(ids, fold_dir_path, 5)

    # 根据树文件生成路径ids和路径texts
    print('根据树文件生成路径ids和路径texts...')
    generate_path_ids_and_texts(tree_dir_path, weibo_all_path_ids_path, weibo_all_path_raw_texts_path)

    # 根据路径ids生成回复路径texts
    print('根据路径ids生成回复texts...')
    get_response_path(weibo_all_path_ids_path, weibo_all_response_raw_texts_path)

    # 预处理路径texts：替换表情包，删除url和@等符号
    print('预处理路径文本...')
    path_lines = load_lst(weibo_all_path_raw_texts_path)
    ...
    # 中文分词
    path_lines = chinese_tokenization(path_lines)
    save_lst(weibo_all_path_texts_path, path_lines)

    # 预处理回复路径texts：替换表情包，删除url和@等符号
    print('预处理回复路径文本...')
    response_lines = load_lst(weibo_all_response_raw_texts_path)
    ...
    # 中文分词
    response_lines = chinese_tokenization(response_lines)
    save_lst(weibo_all_response_texts_path, response_lines)

    # 为WAE预处理路径文本和回复路径文本
    print('预处理path文本...')
    pre4wae_chinese(path_vocab4random, weibo_all_path_texts_path, _args.random_vocab_dim - 1)
    print('预处理response path文本...')
    pre4wae_chinese(response_path_vocab4wae, weibo_all_response_texts_path, _args.response_vocab_dim - 1)
