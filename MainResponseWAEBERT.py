...
# 相比MainResponseWAE：使用BERT直接编码整条路径（path和response path），而不是使用随机初始化
from models.PathBased import ResponseWAE, ResponseCatWAE, ResponseWAEBERT
...
def main():
    ...
    # 整条路径编码：random；bert；glove等
    x_train_random, y_train_random, x_test_random, y_test_random = \
        load_path_data_bert(train_path, test_path, label_path,
                            path_bert_id2paths_dict_path)
    ...
    model = ResponseWAEBERT(_args.random_vocab_dim, _args.response_vocab_dim,
                            wae_best_encoder_path, _args.bert_dim,
                            _args.vae_dim, _args.class_num)
    model.to(device)
